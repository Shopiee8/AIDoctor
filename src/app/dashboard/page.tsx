    ...
    <div className="space-y-6 max-w-[1800px] mx-auto p-4">
          ...
          {dependants.map((dep, i) => (
            <div key={i} className="flex items-center gap-3">
              <Avatar className="w-12 h-12">
                <AvatarImage src={dep.img} alt={dep.name} />
              ...
              <div>
                ...
                <div className="text-xs">{dep.age}</div>
              </div>
            </div>
          ))}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest consultation reports</CardDescription>
        </CardHeader>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {reportsTable.map((report, i) => (
                <tr key={i} className="even:bg-muted">
                  <td className="px-4 py-2">{report.id}</td>
                  <td className="px-4 py-2">{report.doctor}</td>
                  <td className="px-4 py-2">{report.date}</td>
                  <td className="px-4 py-2">{report.type}</td>
                  <td className={`px-4 py-2 font-semibold ${report.status === 'Upcoming' ? 'text-blue-600' : report.status === 'Completed' ? 'text-green-600' : 'text-red-600'}`}>
                    {report.status}
                  </td>
                </tr>
              ))}
